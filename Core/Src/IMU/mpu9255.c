...
float gyroBias[3] = { 0, 0, 0 }, accelBias[3] = { 0, 0, 0 }, magBias[3] = { 0,
		0, 0 }, magScale[3] = { 0, 0, 0 }; // Bias corrections for gyro and accelerometer
float SelfTest[6];    // holds results of gyro and accelerometer self test

float GyroMeasError = PI * (40.0f / 180.0f); // gyroscope measurement error in rads/s (start at 40 deg/s)
float GyroMeasDrift = PI * (0.0f / 180.0f); // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)

//float beta = sqrt(3.0f / 4.0f) * GyroMeasErrorAltr;   // compute beta
...
#ifdef LCD_EN
		lcd_send_string(1,0,"MAG Calib", &lcd1);
#endif
...
//		if (!CHECK_SENS_ERR(siracUydu.donanimDurumu, FLASH_ERROR)) {
//			for (int i = 0; i < 6; i++) {
//				sprintf((char*) magData[i], "%d",
//						i >= 3 ? mag_max[i - 3] : mag_min[i]);
...
//		memset(mag_max, 0, 3 * sizeof(int16_t));
//		memset(mag_min, 0, 3 * sizeof(int16_t));
//		for (int i = 0; i < 6; i++) {
//			status |= (W25qxx_Read(magData[i],
//					(FLASH_ADD_STARTING_FOR_MAG + (i * 8)), 8));
//			if (i >= 3)
//				mag_max[i - 3] = atoi((char*) magData[i]);
//			else
//				mag_min[i] = atoi((char*) magData[i]);
//			printf("%s ", magData[i]);
//		}
//		printf("\n");
//		if (status != HAL_OK)
